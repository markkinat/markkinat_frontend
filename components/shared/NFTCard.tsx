...
const NFTCard = ({ nft }: any) => {
  ...
  return (
      <Link href={`/collection/${nft.address}`} className='z-1'>
                ...
                <div className="mt-3 flex flex-col">
                   <p className={`font-poppins ${theme === 'dark' ? 'text-white' : 'text-nft-black-1'} font-semibold text-sm lg:text-xl`}>{nft.name}
                    ...
                    <div className="flexBetween mt-1 minlg:mt-3 flex-row xs:flex-col xs:items-start xs:mt-3">
                        <p className={`font-poppins ${theme === 'dark' ? 'text-white' : 'text-nft-black-1'} font-semibold text-xs lg:text-lg`}>{nft.price || "0.01"}<span className="font-normal">{nftCurrency}</span></p>
